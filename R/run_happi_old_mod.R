#' Corrected function running happi, p=q=1; this script contains the corrected non-modular implementation of the previous happi function 
#'
...
#'
#' @export
happi_old_mod <- function(outcome,
                  covariate,
                  quality_var,
                  max_iterations = 50,
                  min_iterations = 15,
  ...
  update_f <- function(probs,
    ...
    if(method == "isotone") {
      ...
      if (any(abs(best_b - 100) < 0.01)) suppressWarnings(warning("spline basis weights close to boundaries; try reducing spline_df ",tt))
      if (any(abs(best_b > 1e4))) suppressWarnings(warning("spline basis weights close to boundaries; try reducing spline_df ",tt))
  ...
  incomplete_loglik <- function(xbeta, ff, firth = TRUE, mycovariate) {
    ...
    if (!firth) {
      ...
      ll <-  sum(log( (1 - epsilon) * (1 - prob_lambda[outcome == 0]) +
        ...
        sum(log(epsilon * (1 - prob_lambda[outcome == 1]) +
                  ff[outcome == 1] * prob_lambda[outcome == 1]))
      penalty <- 0.5*msos::logdet(t(mycovariate) %*% diag(as.numeric(prob_lambda)*(
        1 - as.numeric(prob_lambda))) %*% mycovariate)
      return(ll + penalty)
    }
  }
  ...
  while (tt <= max_iterations & keep_going) {
    ...
    if ((tt > min_iterations) & (my_estimates[tt, "loglik"] > my_estimates[tt, "loglik_null"])) {
      ...
      if(!keep_going) {
        check_updated_f <- warningcheck_update_f(probs=my_estimated_p[tt - 1, ],
                                                 method = method,
                                                 spline_df = spline_df)
        check_updated_f_null <- warningcheck_update_f(probs=my_estimated_p_null[tt - 1, ],
                                                      method = method,
                                                      spline_df = spline_df) # add in warning check for the last iteration 
      }
    }
