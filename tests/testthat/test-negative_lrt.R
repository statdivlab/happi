...
test_that("likelihood is higher under alternative than null", {
  ...
  happi_spline_firth <- happi(outcome = ys,
                              covariate = xx,
                              quality_var = mm,
                              max_iterations = 1000,
                              nstarts = 1,
                              ...
                              epsilon = 0)
  happi_spline_logreg <- happi(outcome = ys,
                               covariate = xx,
                               quality_var = mm,
                               max_iterations = 1000,
                               nstarts = 1,
  ...
  happi_isotone_firth <- happi(outcome = ys,
                              covariate = xx,
                              quality_var = mm,
                              max_iterations = 1000,
                              nstarts = 1,
                              ...
                              change_threshold = 0.05,
                              epsilon = 0)
  happi_isotone_logreg <- happi(outcome = ys,
                               covariate = xx,
                               quality_var = mm,
                               max_iterations = 1000,
                               nstarts = 1,
  ...
  expect_gte(tail(happi_spline_firth$loglik$LRT[!is.na(happi_spline_firth$loglik$LRT)], 1), 0)
  expect_gte(tail(happi_spline_logreg$loglik$LRT[!is.na(happi_spline_logreg$loglik$LRT)], 1), 0)
  expect_gte(tail(happi_isotone_firth$loglik$LRT[!is.na(happi_isotone_firth$loglik$LRT)], 1), 0)
  expect_gte(tail(happi_isotone_logreg$loglik$LRT[!is.na(happi_isotone_logreg$loglik$LRT)], 1), 0)
